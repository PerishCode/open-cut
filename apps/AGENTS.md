...
- Product peer identifiers belong in `packages/contracts`; generated HTTP API
  code belongs in `packages/openapi`; shared control-plane mechanics belong in
...
- Web business source is limited to `lib`, `views`, `components`, and `main.tsx`.
  ...
  `components` composes atoms from `packages/components`; only `lib` consumes
  generated API functions from `packages/openapi`.
