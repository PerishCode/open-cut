...
func (repository *SQLiteProjects) CompleteMediaProxy(
	...
	if err := verifyMediaAttempt(ctx, verification, input.Claim, input.CompletedAt.UTC(), domain.MediaJobProxy); err != nil {
...
WHERE asset_id = ? AND kind = 'proxy' AND producer_version = ?
  AND input_fingerprint = ? AND parameters_digest = ?`,
	...
	finalRoot, err := repository.publishProxyFiles(input)
	if err != nil {
		return err
	}
	...
	defer tx.Rollback()
	if err := verifyMediaAttempt(ctx, tx, input.Claim, input.CompletedAt.UTC(), domain.MediaJobProxy); err != nil {
		return err
	}
...
SET state = 'ready'
WHERE id = ? AND project_id = ? AND asset_id = ? AND kind = 'proxy' AND state = 'evicted'
...
WHERE id = ? AND state = 'running'`, at, at, input.Claim.AttemptID.String())
	...
	if err := appendMediaJobActivity(
		...
		"media.proxy-ready", "media-proxy-ready", nil,
	); err != nil {
		return err
	}
...
func validateProxyPublication(input application.CompleteMediaProxy) error {
	if input.EventID.IsZero() || input.ArtifactID.IsZero() || input.CompletedAt.IsZero() ||
		input.Claim.Kind != domain.MediaJobProxy || input.Claim.AcceptedFingerprint == nil ||
		input.Parameters.Validate() != nil || input.Parameters.AssetID != input.Claim.AssetID ||
		input.Parameters.Kind != domain.MediaJobProxy || input.Parameters.Profile != application.SourceProxyProfile ||
		...
		input.Manifest.Producer != input.Claim.ExecutorVersion || input.Workspace == nil ||
		len(input.ManifestCanonical) == 0 || input.ByteSize.Value() > application.MaximumSourceProxyArtifactSize {
		return domain.ErrInvalidMediaFacts
	}
	canonical, digest, err := domain.CanonicalDigest(
		"open-cut/source-proxy-artifact", application.SourceProxyArtifactSchema, input.Manifest,
	)
...
func (repository *SQLiteProjects) publishProxyFiles(
	input application.CompleteMediaProxy,
) (string, error) {
	...
	if err := copyPreparedProxyFile(stageRoot, input.Workspace, input.Manifest.Media); err != nil {
		return "", err
	}
	if input.Manifest.Video != nil {
		if err := copyPreparedProxyFile(stageRoot, input.Workspace, input.Manifest.Video.TimeMap); err != nil {
			return "", err
		}
		...
		validationErr := application.ValidateSourceProxyTimeMapReader(
			file, input.Manifest.Video.FrameCount.Value(),
		)
		closeErr := file.Close()
	...
	if _, err := os.Lstat(finalRoot); !os.IsNotExist(err) {
		return "", fmt.Errorf("proxy artifact destination already exists")
	}
...
func copyPreparedProxyFile(
	stageRoot string,
	workspace application.PreparedMediaWorkspace,
	record application.SourceProxyArtifactFile,
) error {
	...
	if err != nil || uint64(written) != record.ByteSize.Value() ||
		...
		return fmt.Errorf("prepared proxy file failed content verification")
	}
