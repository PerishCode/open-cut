...
func TestRuntimeHostGivesUpOnDeterministicPeerFailure(t *testing.T) {
	executable, err := os.Executable()
	if err != nil {
		t.Fatal(err)
	}
	base := t.TempDir()
	identity, _ := cell.New("test", "runtime-give-up")
	paths, err := layout.Resolve(config.RootSet{
		BootstrapRoot: filepath.Join(base, "bootstrap"), StoreRoot: filepath.Join(base, "store"),
		CacheRoot: filepath.Join(base, "cache"), RuntimeRoot: filepath.Join(base, "runtime"), LogRoot: filepath.Join(base, "logs"),
	}, identity)
	...
	dataDir := filepath.Join(base, "data", identity.Suffix())
	plan := runtimetopology.Plan{Processes: []runtimetopology.ResolvedProcess{{
		App: "failing-peer", Command: executable,
		Args: []string{"-test.run=^TestRuntimeHostHelperProcess$"}, WorkingDirectory: base,
		Env: map[string]string{
			"OC_RUNTIMEHOST_HELPER": "1", "OC_RUNTIMEHOST_COUNTER": counter,
			"OC_RUNTIMEHOST_ALWAYS_FAIL":       "1",
			"OC_RUNTIMEHOST_EXPECTED_DATA_DIR": dataDir,
		},
	}}}
	...
	go func() {
			...
			DataDir: dataDir, Installation: runtimeHostTestInstallation(), App: "runtime",
			Mode: protocol.LifecycleModeHarness, Presentation: protocol.PresentationHeadless, Source: "harness",
			Plan: plan, ReadyTimeout: 20 * time.Second,
		}, ready)
	}()
	select {
	case <-ready:
		t.Fatal("deterministically failing peer reached READY")
	case err := <-done:
		if err == nil || !strings.Contains(err.Error(), "giving up") {
			t.Fatalf("runtime error = %v, want terminal giving-up failure", err)
		}
	case <-ctx.Done():
		t.Fatal(ctx.Err())
	}
	...
	starts, err := strconv.Atoi(strings.TrimSpace(string(startsBytes)))
	if err != nil || starts != consecutiveFailureLimit {
		t.Fatalf("peer start count = %q, want exactly %d", startsBytes, consecutiveFailureLimit)
	}
}
...
func TestRuntimeHostHelperProcess(t *testing.T) {
	...
	if os.Getenv("OC_RUNTIMEHOST_ALWAYS_FAIL") == "1" {
		os.Exit(23)
	}
	if starts == 1 {
		os.Exit(23)
