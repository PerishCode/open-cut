...
func runManagedRelease(
	...
	launchEnvironment, err := protocol.AppendLaunchEnvironment(os.Environ(), protocol.SidecarLaunch{
		...
		Namespace: identity.Namespace, DataDir: bootstrap.DataDir, Installation: bootstrap.Installation, Mode: protocol.LifecycleModePackaged,
...
func RunL1(ctx context.Context, options L1Options) error {
	...
	return runtimehost.Run(ctx, runtimehost.Options{
		...
		Channel: launch.Channel, Namespace: launch.Namespace, DataDir: launch.DataDir, Installation: launch.Installation,
